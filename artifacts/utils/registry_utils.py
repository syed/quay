...
from image.oci import OCI_IMAGE_CONFIG_CONTENT_TYPE, OCI_IMAGE_MANIFEST_CONTENT_TYPE
...
class QuayRegistryClient:
    ...
    def _do_request(self, method, path, headers=None, query_string=None, data=None):
        with app.test_client() as c:
            response = c.open(
                method=method, path=path, headers=headers, data=data, query_string=query_string
            )
            return response

    def check_blob_exists(self, namespace, repo_name, digest, registry_grant_token):
        headers = {"Authorization": f"Bearer {registry_grant_token}"}
    ...
    def upload_artifact_blob(self, namespace, repo_name, data, data_digest, registry_grant_token):
        blob_exists_response = self.check_blob_exists(
            namespace, repo_name, data_digest, registry_grant_token
        )
    ...
    def create_oci_artifact_manifest(
        self, media_type, length, digest, config_layer=None, layers=None
    ):
    ...
    def upload_oci_artifact_manifest(
        self, namespace, repo_name, manifest: OCIArtifactManifest, tag, registry_grant_token
    ):
    ...
    def get_oci_manifest(self, namespace, repo_name, tag_or_digest, grant_token):
        path = f"/v2/{namespace}/{repo_name}/manifests/{tag_or_digest}"
        headers = {
    ...
    def get_oci_blob(self, namespace, repo_name, digest, grant_token, follow_cdn=False):
        ...
        if response.status_code == 200:
            ...
            if response.status_code == 401:
                abort(401, message=response.data)
            setattr(response, "data", response.headers)
            return response
        return response

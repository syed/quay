...
from artifacts.utils.registry_utils import QuayRegistryClient, RegistryError
...
def get_package_metadata(namespace, package_name, tag, grant_token):
    ...
    """
    response = quayRegistryClient.get_oci_manifest(namespace, package_name, tag, grant_token)
...
def get_package_tarball(auth_result, namespace, package_name, package_version):
    ...
    grant_token = generate_auth_token_for_read(auth_result, namespace, package_name)
    response = quayRegistryClient.get_oci_manifest(
        namespace, package_name, package_version, grant_token
    )
    if response.status_code != 200:
        return response

...
from artifacts.plugins.python.constants import (
    PYPI_JSON_CONTENT_TYPE,
    PYTHON_PACKAGE_MEDIA_TYPE,
)
from artifacts.plugins.python.python_utils import (
    get_layer_media_type,
    get_python_packages_for_repo,
    quayRegistryClient,
    update_manifest_with_new_layer,
)
...
def upload_package(auth_result, index):
    ...
    response = quayRegistryClient.upload_artifact_blob(
        namespace, repo_name, data, data_digest, grant_token
    )
    ...
    manifest_response = quayRegistryClient.get_oci_manifest(
        namespace, repo_name, repo_tag, grant_token
    )
    logger.info(
        f"🟦 🟦 🟦 🟦  manifest response {manifest_response.data} {manifest_response.status_code}"
    )
    ...
    response = quayRegistryClient.upload_oci_artifact_manifest(
        namespace, repo_name, manifest, repo_tag, grant_token
    )
...
@bp.route(
    "/download/<namespace>/<repo_name>/<digest>/<filename>", strict_slashes=False, methods=["GET"]
)
@validate_plugin_auth(artifact_plugin_validate_basic_auth)
